...
def pcap_parser_main():
    ...
    get_tuples = str(
        "")  # this will be a list of src_ip:src_port>dst_ip:dst_port that we check responses against before we display them
    # here come the regex!
    http_request_pattern = re.compile(
        r'(?s).{54,94}(GET|POST|PUT|HEAD) (([^/]*)\S*(/\S*)) HTTP/1\.[01]((?:(?:(?:\r\n)|\n)[^:]*: [^\r\n]+)*)(?:(?:\r\n)|\n){2}')  # up to max tcp header
    http_response_pattern = re.compile(
        r'(?s).{54,94}HTTP/1\.[01] (\d+)[^\r\n]*((?:(?:(?:\r\n)|\n)[^:]*: [^\r\n]+)*)(?:(?:\r\n)|\n){2}')  # up to max header
    # ok, no more regex
    frame = 1
    ...
    with open(output_filepath, 'w') as outfile:
        ...
        outfile.write(
            "timeStamp\tframe\tsource_address\tsource_port\tdestination_address\tdestination_port\thost\theadercount\tmethod/code\turl/responsemeta\theaders\tuser-agent/content-type\treferer/content-disposition\n")
        while True:
            http_request_header_results = None
            http_response_header_results = None
            try:
                (header, payload) = reader.next()
                http_request_header_results = re.match(http_request_pattern,
                                                       payload)  # re.match is used to deliberately search only once at the beginning of the string
                ...
                if http_request_header_results:
                    ...
                    if max_header_count_input:  # if the user provided an int, this code block is used. The only difference between this and the "else"...
                        ...
                        if http_request_header_count <= max_header_count_input:
                            ...
                            http_request_headers_combined = str(
                                "\"")  # surround the headers with a double-quote so they can be treated as a single value by excel
                            for i in http_request_headers:  # sanitize headers, put special ones into their own variable, put all into a httpResponseHeadersCombined variable
                                if str(i).startswith("Host:"):
                                    ...
                                    http_request_headers_combined = str(
                                        http_request_headers_combined + str("Host: " + http_request_host) + "\n")
                                elif str(i).startswith("User-Agent:"):
                                    http_user_agent = i.replace("User-Agent: ", "").rstrip("\r\n")
                                    http_request_headers_combined = str(
                                        http_request_headers_combined + str("User-Agent: " + http_user_agent) + "\n")
                                elif str(i).startswith("Referer:"):
                                    http_request_referrer = i.replace("Referer: ", "").replace(".", "[.]").replace(
                                        "http://", "hxxp[:]//").rstrip("\r\n")
                                    http_request_headers_combined = str(
                                        http_request_headers_combined + str("Referer: " + http_request_referrer) + "\n")
                                else:
                                    http_request_headers_combined = str(
                                        http_request_headers_combined + str(i).rstrip("\r\n") + "\n")
                            http_request_headers_combined = str(
                                http_request_headers_combined.rstrip("\n") + "\"")  # remove the final newline...
                            #  and add a double-quote to make the TSV treat all headers as a single value.
                            get_tuples = (
                            get_tuples + src_ip + ":" + tcp_src_port + ">" + dst_ip + ":" + tcp_dst_port)  # add this request's src/dst ip/ports to a list
                            # output to the .TSV - all done with the HTTP request!
                            time_stamp = convert_timefromepoch(header.getts()[0])
                            outfile.write(time_stamp + "\t" + str(
                                frame) + "\t" + src_ip + "\t" + tcp_src_port + "\t" + dst_ip + "\t" + tcp_dst_port + "\t" \
                                          + http_request_host + "\t" + str(
                                http_request_header_count) + "\t" + http_request_method \
                    ...
                    if isinstance(l2, impacket.ImpactPacket.IP):
                        ...
                        if isinstance(l3, impacket.ImpactPacket.TCP):
                            ...
                            tcp_dst_port = str(l3.get_th_dport())
                    reversed_tuple = (
                    dst_ip + ":" + tcp_dst_port + ">" + src_ip + ":" + tcp_src_port)  # reverse this packet's IPs and ports so we
                    ...
                    if reversed_tuple in get_tuples:  # check to see if this response is a reply to a request we wrote to the file...
                        ...
                        http_response_headers_combined = str(
                            "\"")  # surround the headers with a double-quote so they can be treated as a single value by excel
                        for i in http_response_headers:  # extract and sanitize the interesting response headers, put all headers in httpResponseHeadersCombined
                            if str(i).startswith("Content-Type:"):
                                http_response_content_type = str(i).replace("Content-Type: ", "").replace("\"",
                                                                                                          "%22").rstrip(
                                    "\r\n")
                                http_response_headers_combined = str(
                                    http_response_headers_combined + "Content-Type: " + http_response_content_type + "\n")
                            elif str(i).startswith("Content-Disposition:"):
                                http_response_content_disposition = str(i).replace("Content-Disposition: ", "").replace(
                                    "\"", "%22").rstrip("\r\n")
                                http_response_headers_combined = str(
                                    http_response_headers_combined + "Content-Disposition: " + http_response_content_disposition + "\n")
                            elif str(i).startswith("Location:"):
                                http_response_location = str(i).replace("Location: ", "").replace("\"", "%22").replace(
                                    ".", "[.]").replace("http://", "hxxp[:]//").rstrip("\r\n")
                                http_response_headers_combined = str(
                                    http_response_headers_combined + "Location: " + http_response_location)
                            else:
                                http_response_headers_combined = str(
                                    http_response_headers_combined + str(i).replace("\"", "%22").rstrip("\r\n") + "\n")
